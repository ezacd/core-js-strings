...
function isPalindrome(str) {
  const rev = str.toLowerCase().replace(/[\W_]/g, '');

  return rev.split('').reverse().join('') === rev;
}
...
function extractEmails(str) {
  return str.split(';').length ? str.split(';') : str;
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((char) => {
      const charIndex = originalAlpha.indexOf(char);
      return charIndex === -1 ? char : cipher[charIndex];
    })
    .join('');
}
